...
constexpr static bool GKOS_FUNC(is_power_of_2)(uint32_t v)
{
    if(!v)
        return false;
    while(!(v & 0x1UL)) v >>= 1;
...
constexpr static bool GKOS_FUNC(is_multiple_of)(uint32_t num, uint32_t denom)
{
    return (num % denom) == 0;
}
...
template <uint32_t min_buddy_size, uint32_t tot_length, uint32_t base_addr> class BuddyAllocator
{
    ...
    protected:
        ...
        constexpr static uint32_t GKOS_FUNC(level_buddy_size)(uint32_t level_id)
        {
            return min_buddy_size << level_id;
        }

        constexpr static uint32_t GKOS_FUNC(bits_for_level)(uint32_t level_id)
        {
        ...
        constexpr static uint32_t GKOS_FUNC(words_for_level)(uint32_t level_id)
        {
        ...
        constexpr static uint32_t GKOS_FUNC(num_levels)()
        {
            uint32_t nlevels = 1;
        ...
        constexpr static uint32_t GKOS_FUNC(total_words)()
        {
            uint32_t nwords = 0;
        ...
        constexpr static uint32_t GKOS_FUNC(buddy_size_to_level)(uint32_t size)
        {
            return __CLZ(min_buddy_size) - __CLZ(size);
        }

        constexpr static uint32_t GKOS_FUNC(addr_to_bitidx_at_level)(uint32_t level, uint32_t addr)
        {
            return addr / level_buddy_size(level);
        }

        uint32_t GKOS_FUNC(lock)()
        {
        ...
        void GKOS_FUNC(unlock)(uint32_t cpsr)
        {
#if GK_DUAL_CORE | GK_DUAL_CORE_AMP
            set(&_lock_val, 0UL);
            __DMB();
        ...
        void GKOS_FUNC(release_at_level)(uint32_t level, uint32_t bitidx)
        {
        ...
        uint32_t GKOS_FUNC(acquire_at_level)(uint32_t level)
        {
        ...
        uint32_t GKOS_FUNC(get_smallest_buddy_size_for_block)(uint32_t *addr)
        {
    ...
    public:
        void GKOS_FUNC(release)(const BuddyEntry &be)
        {
            auto cpsr = GKOS_FUNC(lock)();

            if(be.valid)
            {
                auto level = GKOS_FUNC(buddy_size_to_level)(be.length);
                GKOS_FUNC(release_at_level)(level,
                    GKOS_FUNC(addr_to_bitidx_at_level)(level, be.base - base_addr));
            }
            else
            {
                ...
                while(true)
                {
                    uint32_t cur_buddy_size = GKOS_FUNC(get_smallest_buddy_size_for_block)(&cur_addr);
                    ...
                    auto level = GKOS_FUNC(buddy_size_to_level)(cur_buddy_size);
                    GKOS_FUNC(release_at_level)(level, GKOS_FUNC(addr_to_bitidx_at_level)(level, cur_addr));

                    cur_addr += cur_buddy_size;
                }
            }

            GKOS_FUNC(unlock)(cpsr);
        }

        BuddyEntry GKOS_FUNC(acquire)(uint32_t length)
        {
            BuddyEntry ret;

            // round up to a buddy size
            if(!GKOS_FUNC(is_power_of_2)(length))
            {
            ...
            auto level = GKOS_FUNC(buddy_size_to_level)(length);

            auto cpsr = GKOS_FUNC(lock)();
            auto bitret = GKOS_FUNC(acquire_at_level)(level);
            GKOS_FUNC(unlock)(cpsr);
        ...
        void GKOS_FUNC(init)()
        {
            ...
            for(uint32_t i = 0; i < num_levels(); i++)
            {
                ...
                auto word_count = GKOS_FUNC(words_for_level)(i);
                level_word_counts[i] = word_count;
                cur_start += word_count;
            }
        }

        constexpr uint32_t GKOS_FUNC(MinBuddySize)() { return min_buddy_size; };
};
