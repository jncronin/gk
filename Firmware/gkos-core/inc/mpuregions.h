...
static constexpr uint32_t GKOS_FUNC(align_up)(uint32_t v)
{
    v--;
    v |= v >> 1;
    v |= v >> 2;
...
constexpr mpu_saved_state GKOS_FUNC(MPUGenerate)(uint32_t base_addr,
    uint32_t length,
    uint32_t reg_id,
    bool executable,
    ...
    uint32_t tex_scb)
{
    ...
    length = GKOS_FUNC(align_up)(length);
    base_addr &= ~(length - 1);
    length = GKOS_FUNC(align_up)(end - base_addr);
...
constexpr mpu_saved_state GKOS_FUNC(MPUGenerateNonValid)(uint32_t reg_id)
{
    mpu_saved_state ret { 0, 0 };
...
constexpr mpu_saved_state mpu_sram = GKOS_FUNC(MPUGenerate)(0x30000000, 0x10000000, 0, false, RW, RO, N_NC_S);
constexpr mpu_saved_state mpu_fb0 = GKOS_FUNC(MPUGenerate)(0x60000000, 0x400000, 1, false, RW, RW, WT_NS);

constexpr mpu_saved_state mpu_default[8] =
{
    mpu_sram,
    mpu_fb0,
    GKOS_FUNC(MPUGenerateNonValid)(2),
    GKOS_FUNC(MPUGenerateNonValid)(3),
    GKOS_FUNC(MPUGenerateNonValid)(4),
    GKOS_FUNC(MPUGenerateNonValid)(5),
    GKOS_FUNC(MPUGenerateNonValid)(6),
    GKOS_FUNC(MPUGenerateNonValid)(7)
};
