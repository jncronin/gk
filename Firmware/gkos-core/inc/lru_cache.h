        ...
        class GKOS_FUNC(LRUCache)
{
    ...
    public:
        GKOS_FUNC(LRUCache)(size_t max_size)
        {
            _max_size = max_size;
        }
...
template<typename Key, typename T, int region_id> using GKOS_FUNC(LRUCache)Region = 
    GKOS_FUNC(LRUCache)<Key, T, std::hash<Key>, std::equal_to<Key>,
