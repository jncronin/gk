...
static char *myfgets(char *buf, int num, void *str)
{
    ...
    auto br = deferred_call(syscall_read, (int)str, myfgets_buffer, blen - 1);
    if(br <= 0)
        return nullptr;
    ...
    if(first_nline)
    {
        ...
        deferred_call(syscall_lseek, (int)str, -backtrack_by, SEEK_CUR);
    }
...
{
    if(last_update.is_valid() && clock_cur() < (last_update + kernel_time::from_ms(500)))
    {
        return known_nets;
    }
    ...
    auto fd = deferred_call(syscall_open, "/etc/wifi.secrets", O_RDONLY, 0);
    if(fd < 0)
    {
