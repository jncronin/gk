				...
				struct usb_setup_req *req)
{
	...
	case USBD_STATE_ADDRESSED:
		if (cfgidx != 0U) {
			...
			if (pdev->_class->init(pdev, cfgidx) != 0U) {
				usb_core_ctl_error(pdev);
				return;
			}
		}
	...
	case USBD_STATE_CONFIGURED:
		if (cfgidx == 0U) {
			...
			pdev->dev_config = cfgidx;
			pdev->_class->de_init(pdev, cfgidx);
			...
			pdev->_class->de_init(pdev, pdev->dev_config);
	...
	if (epnum == 0U) {
		...
		if (pdev->ep0_state == USBD_EP0_DATA_OUT) {
			if (pep->rem_length > pep->maxpacket) {
				...
				if (pdev->_class->ep0_rx_ready &&
				    (pdev->dev_state ==
				     USBD_STATE_CONFIGURED)) {
					pdev->_class->ep0_rx_ready(pdev);
				}

				usb_core_transmit_ep0(pdev, NULL, 0U);
			}
		}
	} else if (pdev->_class->data_out != NULL &&
		   (pdev->dev_state == USBD_STATE_CONFIGURED)) {
		pdev->_class->data_out(pdev, epnum);
	}
					...
					uint8_t *pdata)
{
		...
		if (pdev->ep0_state == USBD_EP0_DATA_IN) {
			if (pep->rem_length > pep->maxpacket) {
				...
				if ((pep->total_length % pep->maxpacket ==
					...
					if (pdev->_class->ep0_tx_sent != NULL &&
					    (pdev->dev_state ==
					     USBD_STATE_CONFIGURED)) {
						pdev->_class->ep0_tx_sent(pdev);
					}
					/* Start the transfer */
	...
	} else if ((pdev->_class->data_in != NULL) &&
		   (pdev->dev_state == USBD_STATE_CONFIGURED)) {
		pdev->_class->data_in(pdev, epnum);
	}
...
{
	if (pdev->dev_state == USBD_STATE_CONFIGURED) {
		if (pdev->_class->sof != NULL) {
			pdev->_class->sof(pdev);
		}
	}
