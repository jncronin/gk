...
{
    osd.has_osd = true;
    osd.osd_prepped = false;
    osd.osd = nullptr;
    osd.osd_text = _osd_text;
}
...
bool anim_handle_quit_failed(Widget *wdg, void *p, time_ms_t t)
{
    ...
    auto pid = (id_t)(uintptr_t)p;
    auto proc = ProcessList.Get(pid).v;
    
    if(proc)
    {
        klog("supervisor: request to quit pid %u failed, force-quitting\n", pid);

        Process::Kill(pid, 128 + SIGKILL);
    }
...
static char *strreader(char *ret, int num, void *stream)
{
    ...
    auto toread = std::min(maxread, (long)num-1);
...
{
    if(!osd.has_osd || !osd.osd_prepped || !osd.osd)
        return;
    
    delete_children_and_self(osd.osd);
}
...
Widget *Process::get_osd()
{
    ...
    if(!osd.osd_prepped)
    {
        ...
        ini_str_provider iprov { .buf = (char *)osd.osd_text.c_str(), .pos = 0 };
...
void key_handler(Widget *w, coord_t x, coord_t y)
{
    ...
    if(w->d && focus_process)
    {
        ...
        if(l->is_quit)
        {
            ...
            AddAnimation(*GetAnimationList(), clock_cur_ms(), anim_handle_quit_failed, nullptr, (void *)focus_process->id);
        }
    }
}
