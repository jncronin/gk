...
static void disable_phy_ddc(void)
{
	...
	mmio_write_32(DDRPHYC_BASE + DDRPHY_APBONLY0_MICROCONTMUXSEL, 0U);
	mmio_write_32(DDRPHYC_BASE + DDRPHY_DRTUB0_UCCLKHCLKENABLES,
		      DDRPHY_DRTUB0_UCCLKHCLKENABLES_UCCLKEN |
		      DDRPHY_DRTUB0_UCCLKHCLKENABLES_HCLKEN);

	/* Disable DRAM drift compensation */
	mmio_write_32(DDRPHYC_BASE + DDRPHY_INITENG0_P0_SEQ0BDISABLEFLAG6, 0xFFFFU);

	/* Disable APB access to internal CSR registers */
	mmio_write_32(DDRPHYC_BASE + DDRPHY_DRTUB0_UCCLKHCLKENABLES,
		      DDRPHY_DRTUB0_UCCLKHCLKENABLES_HCLKEN);
	mmio_write_32(DDRPHYC_BASE + DDRPHY_APBONLY0_MICROCONTMUXSEL,
...
void ddr_wait_lp3_mode(bool sr_entry)
{
	...
	timeout = timeout_init_us(DDR_TIMEOUT_US_1S);
	do {
		uint16_t phyinlpx = mmio_read_32(DDRPHYC_BASE +
	...
	mmio_write_32(DDRPHYC_BASE + DDRPHY_DRTUB0_UCCLKHCLKENABLES, 0U);
...
static int sr_loop(bool is_entry)
{
	 ...
	 */
	do {
		type = mmio_read_32(DDRC_BASE + DDRCTRL_STAT) &
		       DDRCTRL_STAT_SELFREF_TYPE_MASK;
#if STM32MP_LPDDR4_TYPE
		state = mmio_read_32(DDRC_BASE + DDRCTRL_STAT) &
...
{
	uintptr_t ddrctrl_base = DDRC_BASE;
...
static int sr_ssr_entry(bool standby)
{
	...
	if (standby) {
		mmio_clrbits_32(PWR_BASE + PWR_CR11, PWR_CR11_DDRRETDIS);
	}
...
static int sr_hsr_set(void)
{
	...
	mmio_clrsetbits_32(RCC_BASE + RCC_DDRITFCFGR,
			   RCC_DDRITFCFGR_DDRCKMOD_Msk, 2U << RCC_DDRITFCFGR_DDRCKMOD_Pos);
...
{
	mmio_write_32(RCC_BASE + RCC_DDRCPCFGR, RCC_DDRCPCFGR_DDRCPLPEN);
...
{
	mmio_write_32(RCC_BASE + RCC_DDRCPCFGR,
		      RCC_DDRCPCFGR_DDRCPLPEN | RCC_DDRCPCFGR_DDRCPEN);
...
{
	uint32_t pwrctl = mmio_read_32(DDRC_BASE + DDRCTRL_PWRCTL);
