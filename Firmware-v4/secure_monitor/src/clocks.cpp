...
uintptr_t _clocks_cur_s_address()
{
    if(clock_block_vaddr == 0)
        ...
        return clock_block_vaddr + 0xfe00;
}
...
void clock_takeover()
{
    ...
    clock_block_paddr = pmem_vaddr_to_paddr(cb_vaddr, true, true, 3, MT_DEVICE, 0x0e0b0000);

    *(volatile uint64_t *)(cb_vaddr + 0xfe00) = *(volatile uint64_t *)(0x0e0bfe00);
    *(volatile uint64_t *)(cb_vaddr + 0xfe00 + 8) = *(volatile uint64_t *)(0x0e0bfe00 + 8);
